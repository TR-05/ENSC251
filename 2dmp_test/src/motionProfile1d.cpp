...
MotionProfile1d::MotionProfile1d(double distance, double maxVelocity, double acel, double decel) {
    ...
    while (currentTime < totalTime) {
        if (currentTime < acelTime) {
            ...
            currentVelocity = maxVelocity - decel * (currentTime - acelTime - cruiseTime);
        }
